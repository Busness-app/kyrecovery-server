...
// 11. KySecurity Suite App Settings Cards Showcase
const productCardData = {
  kysignon: {
    name: 'KySignOn SSO Server',
    badge: 'Auth & Directory',
    files: 'data/kysignon.db, keys/token_signing_rsa.key',
    deps: 'KY_ISSUER, KY_ADMIN_EMAIL, PORT_8080',
    snippet: `<!-- Add to KySignOn Admin Settings Page -->
<div class="card p-4">
  <h4>KyRecovery Vault & Disaster Sync</h4>
  <p class="text-muted">Protects KySignOn user accounts and RSA token signing keys with automated verification drills.</p>
  <div class="row g-2">
    <div class="col"><input type="text" class="form-control" placeholder="6-digit Pairing Code" id="kyrec-pin"></div>
    <div class="col-auto"><button class="btn btn-primary" onclick="claimKyRecovery('https://recovery.internal:8080')">Pair with KyRecovery</button></div>
  </div>
</div>`
  },
  kypassword: {
    name: 'KyPassword Vault Server',
    badge: 'Secret Vaults',
    files: 'data/kypassword.db, keys/vault_ecdsa.key',
    deps: 'KY_VAULT_SALT, PORT_8081',
    snippet: `// Go backend hook for KyPassword:
import "kyrecovery-server/pkg/client"

func SyncVaultToRecovery(ctx context.Context, serverURL, token, vaultDir string) error {
    c := client.NewClient(serverURL, token)
    _, err := c.PushDirectory(ctx, "kypassword", "KyPassword Primary", "v1.0.0", vaultDir, 2, 3)
    return err
}`
  },
  kybookmarks: {
    name: 'KyBookmarks Server',
    badge: 'Bookmarks & Sync',
    files: 'data/bookmarks.db, config/settings.json',
    deps: 'PORT_8082, KY_BOOKMARKS_SECRET',
    snippet: `<!-- Add to KyBookmarks Preferences / Sync View -->
<div class="sync-card">
  <h3>KyRecovery Backup & Drill Verification</h3>
  <input id="sync-pin" placeholder="Enter KyRecovery PIN" />
  <button onclick="pairBookmarksSync()">Enable Continuous Recovery</button>
</div>`
  },
  kynotes: {
    name: 'KyNotes Server',
    badge: 'Encrypted Notes',
    files: 'data/notes.db, keys/jwt_notes_rsa.key, config/kynotes.json',
    deps: 'PORT_8083, KY_NOTES_KEY_DERIVATION',
    snippet: `// KyNotes Automated Backup Routine:
c := client.NewClient("http://recovery.internal:8080", os.Getenv("KYREC_TOKEN"))
res, err := c.PushDirectory(ctx, "kynotes", "KyNotes Cluster", "v1.2.0", "/var/lib/kynotes", 2, 3)
if err == nil {
    log.Printf("Verified backup capsule: %s", res.CapsuleID)
}`
  },
  kypost: {
    name: 'KyPost Mail Server',
    badge: 'Mail Queues & DKIM',
    files: 'data/mail.db, keys/dkim_rsa.key, config/mail.json',
    deps: 'PORT_8084, PORT_8085, KY_POST_DOMAIN',
    snippet: `# Docker Compose Sidecar for KyPost automated backup sync
services:
  kypost-recovery-sidecar:
    image: kysecurity/kyrecovery:latest
    command: ["kyrecovery", "pair", "push", "--server", "http://kyrecovery:8080", "--token", "\${KYREC_TOKEN}", "--service", "kypost", "--dir", "/var/lib/kypost"]
    volumes:
      - /var/lib/kypost:/var/lib/kypost:ro
    restart: unless-stopped`
  },
  generic: {
    name: 'Custom Business.app Service',
    badge: 'Zero-Code Adapter',
    files: 'data/*.db, config/*.json, certs/*.crt',
    deps: 'APP_PORT, APP_SECRET',
    snippet: `curl -X POST http://localhost:8080/api/backup/push \\
  -H "Authorization: Bearer kyrec_live_token..." \\
  -H "Content-Type: application/json" \\
  -d '{
    "service_name": "my-service",
    "app_name": "Billing API Node 1",
    "threshold": 2,
    "total_shares": 3,
    "files": {
      "data/app.db": "bW9jay1kYXRhYmFzZS1jb250ZW50..."
    }
  }'`
  }
};

let currentSelectedProduct = 'kysignon';

function switchProductCard(serviceKey) {
  currentSelectedProduct = serviceKey;
  const data = productCardData[serviceKey] || productCardData.generic;

  // Update tab styles
  document.querySelectorAll('.product-card-tab').forEach(btn => {
  ...
  document.getElementById('preview-app-badge').textContent = data.name;

  const previewEl = document.getElementById('product-settings-card-preview');
  previewEl.innerHTML = `
    <div style="border-bottom: 1px solid var(--border-color); padding-bottom: 12px; margin-bottom: 14px; display: flex; justify-content: space-between; align-items: center;">
      <div>
        <strong style="color: #fff; font-size: 14px;">KyRecovery Disaster Vault & Backup Sync</strong>
        <div style="font-size: 11px; color: var(--text-dim); margin-top: 2px;">Self-declaring connector for ${data.name}</div>
      </div>
      <span class="status-pill ready"><span class="dot"></span> READY</span>
    </div>

    <div style="font-size: 12px; color: var(--text-dim); margin-bottom: 12px;">
      <div><strong>Protected Files:</strong> <code>${data.files}</code></div>
      <div style="margin-top: 4px;"><strong>Verified Dependencies:</strong> <code>${data.deps}</code></div>
    </div>

    <div style="display: flex; gap: 8px; margin-top: 16px;">
      <input type="text" class="form-input" style="flex: 1; font-family: var(--font-mono); font-size: 12px;" placeholder="6-digit Pairing Code" value="790022" />
      <button class="btn btn-primary btn-sm" onclick="alert('Demo Simulator: Claiming pairing code for ${data.name} and initiating first ephemeral restore drill!')">⚡ Pair & Enable Sync</button>
    </div>

    <div style="margin-top: 14px; display: flex; justify-content: space-between; align-items: center; font-size: 11px; color: var(--text-dim); border-top: 1px solid var(--border-color); padding-top: 10px;">
      <span>Auto-Drill Cadence: <strong style="color: var(--accent-cyan);">Every 6 Hours</strong></span>
      <span>Quorum: <strong style="color: var(--accent-green);">2-of-3 Shares</strong></span>
    </div>
  `;

  // Update code snippet
  document.getElementById('product-code-snippet').textContent = data.snippet;
}

function copyProductSnippet() {
  const snippet = document.getElementById('product-code-snippet').textContent;
  navigator.clipboard.writeText(snippet).then(() => {
    alert('✓ Copied drop-in snippet to clipboard!');
  }).catch(() => {
...
document.addEventListener('DOMContentLoaded', () => {
  if (document.getElementById('product-settings-card-preview')) {
    switchProductCard('kysignon');
  }
});
